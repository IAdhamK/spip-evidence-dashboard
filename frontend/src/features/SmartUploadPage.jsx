    ...
    <section className="smart-upload-page">
      ...
      <div className="smart-upload-header">
        ...
        <div className="smart-upload-mode">
          ...
          />
          <small>{config?.analysis_pipeline_v2_enabled
            ? (config?.advanced_rag?.enabled
              ? `Analisis V2 · Advanced RAG${config?.advanced_rag?.deepseek_enabled ? ` + ${config?.advanced_rag?.model || "DeepSeek V4 Pro"}` : " lokal"}`
              : "Analisis V2 · hasil diperiksa sebelum disimpan")
            : `${config?.ai_provider || "sumopod"} · ${config?.ai_model || "deepseek-v4-pro"} · ${config?.ai_configured ? "API tersambung" : "API belum tersambung"} · ${config?.require_ai ? "AI wajib" : "hybrid"}`}</small>
          {!config?.analysis_pipeline_v2_enabled ? (
