...
Baseline: worktree aktif per 15 Juli 2026
...
Pembaruan 15 Juli 2026:

Migration V32 menambahkan `document-family-v1` sebagai hard gate setelah coverage/template dan sebelum retrieval. Adaptive XLSX screening menginventarisasi seluruh sheet lalu membaca sheet struktural yang relevan meskipun berada setelah empat sheet pertama. Registry family membatasi parameter secara deterministik, role dokumen membatasi authority fakta, confidence keputusan dipisahkan dari skor retrieval/mapping, dan Grade eligibility menahan family tidak relevan, coverage rendah, ambiguity, serta template. UI tidak lagi memakai Grade E sebagai fallback. Rule yang lolos tetap `direction_only` sampai seluruh Independent Verification terkait lulus; hanya hasil tersebut yang menjadi `supported`. Evaluasi sintetis mencapai 100% family accuracy, Top-5 100%, Top-1 100%, false-primary 0%, dan overgrade non-grade 0%; 292 backend tests serta 56 subtests lulus. Kalibrasi pada minimal 50 dokumen berlabel ahli tetap menjadi gate sebelum shadow/pilot.
...
| F1 Persistence/Security | Implementasi kuat | Migration V1–V32, termasuk artefak Advanced RAG dan field auditable family/raw retrieval/calibrated confidence/Grade gate; immutable evaluation/release authority, partition Evaluasi/Learning, evidence-role/template, audit, controlled upload, payload lifecycle, private SQLite, signed attestation, checkpoint, security guard, trusted identity+role, dan legacy backfill fail-closed | Membuktikan SSO/group mapping pada environment nyata, menerbitkan attestation encrypted-volume dari platform produksi, dan storage terdistribusi bila deployment multi-host |
...
| F3 Parameter Retrieval | Implementasi kuat | Document Family hard scope membuang parameter inkompatibel sebelum hybrid/Advanced RAG; risk matrix, monitoring, policy, audit, transmittal, invitation, photo, template, minutes, dan unknown memiliki registry auditable. BM25/cosine-IDF/RRF, abstention, expansion, constrained model rerank, dan leakage-safe Learning feedback tetap tersedia tanpa otoritas Grade | Mengisi dan mengkalibrasi 50/200 holdout Evaluasi serta korpus Learning terpisah; memperluas registry hanya dari bukti domain yang direview |
...
| F8 Tests/Evals | Implementasi kuat | 292 backend tests + 56 subtests lulus. Regression baru mencakup sepuluh family, filename-only negative gate, adaptive XLSX sheet ke-6, pending relevant-sheet gate, substantive/template, transmittal relationship, hard scope, Top-1/Top-5 sintetis, role inheritance, low coverage, ambiguity, Grade block/finalization setelah verification, dua pipeline kasus utama, serta exact Migration V32 validator. Seluruh acceptance, security, queue, OCR, provider, evaluation, handover, dan legacy regression lama tetap lulus | Menyelesaikan review manusia dan holdout Evaluasi 50/200; hasil sintetis 100% belum menggantikan kalibrasi dokumen nyata |
