...
7. Confirm metrics show no high-severity security finding, parser failure spike, stale controlled-upload reservation, atau ambiguity upload terbuka. Production-profile validator v8 harus membuktikan identity+role proxy boundary, runtime RBAC enforcement, exact Migration V32, schema/index evidence-role, expected-template, Document Family Gate/confidence/Grade gate, index idempotensi, trigger ledger append-only, `integrity_check=ok`, `stale_controlled_upload_reservation_count=0`, dan `unresolved_controlled_upload_ambiguity_count=0`.
...
- Disable `VISION_ANALYSIS_ENABLED`, `ANALYSIS_STRUCTURED_MODEL_ENABLED`, `ANALYSIS_ADVANCED_RAG_DEEPSEEK_ENABLED`, `ANALYSIS_MAPPING_REASONING_ENABLED`, or `ANALYSIS_MODEL_VERIFIER_ENABLED` as appropriate. Advanced RAG lokal (filename-aware BM25, cosine-IDF, semantic vector, RRF, dan candidate diversity) tetap berjalan ketika DeepSeek dimatikan. Jika job mengizinkan AI eksternal, DeepSeek `deepseek-v4-pro` menelusuri katalog lengkap KK–subunsur–parameter melalui Chat Completions, lalu constrained reranking membandingkan maksimal sepuluh kandidat; query expansion hanya fallback ketika catalog search tidak menghasilkan kunci valid. Kegagalan provider kembali ke retrieval lokal dan menahan kandidat meragukan sebagai `needs_review`; tidak ada fallback yang mempromosikan mapping atau Grade.
