...
|---|---|---|---|
| 1. Persistence and lifecycle | Forward-only schema, payload retention, worker/shadow/legacy-usage/retrieval-feedback ledger, fact evidence-role provenance, expert template expectation, Document Family/confidence/Grade gate artefacts, controlled-upload reservation/reconciliation, application-owned worker lifespan | `backend/app/migrations.py`, `backend/app/analysis/repository.py`, `backend/app/analysis/payload_storage.py`, `backend/app/lifecycle.py`, `backend/app/main.py`, `scripts/analysis_db_backup.py` | Migration V1–V32 pada database baru; family/confidence/grade-gate, evidence-role/template, controlled-upload unique-key/concurrency/two-person append-only reconciliation; `integrity_check=ok`; paired restore; start/stop/error-cleanup regression; Python warning gate |
...
| 3. Multi-engine analysis | Intake, parsing, OCR/vision, Document Family, fact-role inheritance, family-scoped retrieval, calibrated decision confidence, Grade eligibility, constrained mapping reasoning, grading, verification | `backend/app/analysis/orchestrator.py`, `document_family.py`, `document_family_registry.py`, `routing.py`, `mapping_reasoning.py`, `provider.py`, `document_map.py`, `processors/native.py`, `local_ocr.py`, `vision.py`, `facts.py`, `domain/` | Adaptive XLSX sheet-six fixture; substantive/template split; family-before-retrieval trace; hard scope; low-coverage/ambiguity/role inheritance gates; dua full-pipeline regression; model demotion-only dan deterministic-verifier no-override tests |
