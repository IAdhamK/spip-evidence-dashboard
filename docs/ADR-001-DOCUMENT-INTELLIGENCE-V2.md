...
The application uses a versioned multi-engine pipeline rather than one prompt or one model. Intake/security, routing, native parsing, selective OCR, document mapping, coverage, fact extraction, parameter retrieval, mapping, deterministic grade rules, independent verification, cross-document synthesis, human review, and controlled upload remain separate auditable stages.
...
Parameter retrieval uses `advanced-rag-v1`: parameter-only BM25, cosine-IDF, conservative SPIP/administrative semantic vectors, expert-gold feedback, and reciprocal-rank fusion. Grade criteria are excluded from the retrieval corpus. When local candidates are weak or ambiguous, DeepSeek V4 Pro may create bounded query paraphrases; only terms compatible with the official parameter catalog are accepted. The same constrained adapter may rerank known candidate keys and demote doubtful candidates. It cannot create a parameter, change `mapping_score`, promote status, select a Grade, bypass verification, or authorize upload. A provider outage falls back to local retrieval and fail-closed review.
